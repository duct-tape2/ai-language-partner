...
- Contributor call discussion update:
  `scripts/post_contributor_call_update.py` updates discussion #55 with the
  current first-PR routes and contributor-count status
...
- Contributor call update workflow:
  `.github/workflows/contributor-call-update.yml`
