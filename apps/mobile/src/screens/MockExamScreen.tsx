...
import { MOCK_EXAM, PASS_RATIO, SECTION_LABEL } from '../exam/examData';
import type { ExamSection } from '../exam/examData';

// JLPT N4 모의고사. Intro -> one question at a time (section badge + prompt/passage +
// 4 choices) -> answer feedback with 해설 -> final score with 합격/불합격 + section
// breakdown. Local UI state only; no navigation wiring here.
...
export function MockExamScreen({ app }: { app: AppController }) {
  ...
  const q = MOCK_EXAM[index];
  const answered = picked !== null;
  const isLast = index >= total - 1;

  const start = () => {
    app.track('mock_exam_started', { total });
    setAnswers([]);
    setIndex(0);
    setPicked(null);
    setPhase('quiz');
  ...
  const next = () => {
    ...
    if (isLast) {
      const score = nextAnswers.reduce((s, a, i) => s + (a === MOCK_EXAM[i].answerIndex ? 1 : 0), 0);
      app.track('mock_exam_finished', { score });
      setPhase('result');
    } else {
      setIndex(index + 1);
      setPicked(null);
    ...
    return (
      <View>
        <Fade>
          ...
          <Muted>실제 JLPT N4 형식으로 구성한 {total}문항 모의고사입니다.</Muted>
        </Fade>
        <Fade delay={60}>
          <Card style={{ borderColor: accent, borderWidth: 1.5 }}>
            ...
            <Muted>합격 기준: {Math.round(PASS_RATIO * 100)}% 이상 ({Math.ceil(total * PASS_RATIO)}/{total})</Muted>
  ...
  // ---------- result ----------
  if (phase === 'result') {
    const score = answers.reduce((s, a, i) => s + (a === MOCK_EXAM[i].answerIndex ? 1 : 0), 0);
    const pct = Math.round((score / total) * 100);
    const passed = score / total >= PASS_RATIO;

    const bySection = (['vocab', 'grammar', 'reading'] as ExamSection[]).map((sec) => {
      const idxs = MOCK_EXAM.map((item, i) => ({ item, i })).filter((x) => x.item.section === sec);
        ...
        <Fade delay={60}>
          ...
          >
            ...
            <Text style={{ fontSize: 16, fontWeight: '700', color: theme.colors.subtext, marginTop: 2 }}>
              정답률 {pct}% (합격 {Math.round(PASS_RATIO * 100)}%)
            </Text>
          </Card>
        </Fade>
        <Fade delay={120}>
          <Card>
            ...
            {bySection.map((b) => (
              ...
              >
                <Text style={{ fontSize: 15, fontWeight: '700', color: theme.colors.text }}>{SECTION_LABEL[b.sec]}</Text>
  ...
  return (
    <View>
      <Fade>
        ...
        <Row>
          <Pill label={SECTION_LABEL[q.section]} color={secColor} />
        </Row>
      </Fade>
